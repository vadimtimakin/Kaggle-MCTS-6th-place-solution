...
class OpenFE:
    ...
    def fit(self,
            data: pd.DataFrame, label: pd.DataFrame, group: pd.DataFrame,
            task: str = None,
            train_index=None,
            val_index=None,
    ...
    def get_index(self, train_index, val_index):
        if train_index is None or val_index is None:
            if self.task == 'classification':
                ...
                cv = StratifiedGroupKFold(n_splits=2, shuffle=True, random_state=self.seed)

                for (train_index, test_index) in cv.split(self.data, self.label.astype(str), self.group):
    ...
    def get_init_score(self, init_scores, use_train=False):
        if init_scores is None:
            ...
            if self.feature_boosting:
                ...
                skf = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=self.seed)
                for train_index, val_index in skf.split(data, label, group):
                    ...
                    gbm.fit(X_train, y_train.values.ravel(),
                            eval_set=[[X_val, y_val.values.ravel()]], callbacks=[lgb.early_stopping(200, verbose=False)], verbose=False)
    ...
    def stage2_select(self):
        ...
        if self.task == 'classification':
            ...
            gbm = lgb.LGBMRegressor(**params)
        gbm.fit(train_x, train_y.values.ravel(), init_score=np.array(train_init).ravel(),
                eval_init_score=[np.array(val_init).ravel()],
                eval_set=[(val_x, val_y.values.ravel())],
                callbacks=[lgb.early_stopping(50, verbose=False)], verbose=False)
        results = []
        ...
        try:
            ...
            if self.stage1_metric == 'predictive':
                ...
                gbm.fit(train_x, train_y.values.ravel(), init_score=np.array(train_init).ravel(),
                        eval_init_score=[np.array(val_init).ravel()],
                        eval_set=[(val_x, val_y.values.ravel())],
                        callbacks=[lgb.early_stopping(3, verbose=False)], verbose=False)
        ...
        with ProcessPoolExecutor(max_workers=self.n_jobs) as ex:
            # with tqdm(total=n) as progress:
                for i in range(n):
                    ...
                    #                        train_idx, val_idx)
                    if i == (n - 1):
                        future = self._calculate_multiprocess(candidate_features[i * length:], train_idx, val_idx)
                    else:
                        future = self._calculate_multiprocess(candidate_features[i * length:(i + 1) * length], train_idx, val_idx)
        ...
        with ProcessPoolExecutor(max_workers=self.n_jobs) as ex:
            # with tqdm(total=n) as progress:
                for i in range(n):
                    ...
                    #                              train_idx, val_idx)
                    if i == (n-1):
                        future = self._calculate_and_evaluate_multiprocess(candidate_features[i * length:], train_idx, val_idx)
                    else:
                        future = self._calculate_and_evaluate_multiprocess(candidate_features[i * length:(i + 1) * length], train_idx, val_idx)
                   # future.add_done_callback(lambda p: progress.update())
                    results.append(future)
                res = []
                for r in results:
