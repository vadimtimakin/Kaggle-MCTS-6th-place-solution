...
class Node(object):
    ...
    def calculate(self, data, is_root=False):
        if self.name in all_operators+num_operators:
            ...
            d1 = self.children[0].calculate(data).astype(float)
            d2 = self.children[1].calculate(data).astype(float)
            if self.name == "max":
            ...
            if self.name == "GroupByThenMin":
                ...
                new_data = pd.Series(temp, index=d1.index).astype("str")
...
class FNode(object):
    ...
    def get_fnode(self):
        return [self.name.replace('src_', '').replace('agent1', 'p1_agent').replace('agent2', 'p2_agent')]

    def calculate(self, data):
        self.data = data[self.name.replace('src_', '').replace('agent1', 'p1_agent').replace('agent2', 'p2_agent')]
        return self.data
