...
def transform(X_train, X_test, new_features_list, valid_features, n_jobs, name=""):
    ...
    for feature in tqdm(new_features_list):
        r = ex.submit(_cal, feature, n_train)
        formula = tree_to_formula(feature)
        mask.append("index" in formula)
    ...
    if valid_features is None: valid_features = [*range(len(new_features_list))]
